...
Follow the Ingest procedure in `wiki/SHANK2.wiki/SCHEMA_SHANK2.md`. Pointers:
...
7. Update `index_SHANK2.md` under the "Source summaries" category.
8. Append a `## [YYYY-MM-DD] ingest | Source title` entry to `log_SHANK2.md`. The first bullet is the attribution line `- by: <name> via claude-code`, where `<name>` is the output of `git config user.name` in the wiki repo (read it, do not invent it). Then 2 to 5 bullets describing the ingest. See "Log Entry Attribution" in `SCHEMA_SHANK2.md`.
